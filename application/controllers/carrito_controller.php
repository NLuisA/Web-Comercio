...
class carrito_controller extends CI_Controller {
	...
	public function electrodomesticos()
	{
		...
		if ($session_data)
		{
			...
			$this->load->view('carritoParte',$dat);
		}
	...
	function actualiza_carrito()
    {
		...
		foreach( $cart_info as $id => $cart)
		{
	    	...
	    	$cantidad= $data['qty'];//cantidad al actualizar el producto
	    	...
	    	if ($stock_producto < $cantidad or $cantidad < 0) {
	    		 echo "No puede ingresar Cantidad Mayor que el Stock Disponible";
	    	}else{
	    		$this->cart->update($data);
	    	}
